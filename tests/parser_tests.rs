...
#[test]
fn type_any() {
    let stmts = parse("func f(a: Any) {}");
    match &stmts[0] {
        Stmt::FuncDef { params, .. } => {
            assert_eq!(params[0].ty, Some(Type::Base(BaseType::Any)));
        }
        _ => panic!("expected FuncDef"),
    }
}
...
#[test]
fn type_array_of_any() {
    let stmts = parse("func f(a: Array[Any]) {}");
    match &stmts[0] {
        Stmt::FuncDef { params, .. } => {
            assert_eq!(params[0].ty, Some(Type::Array(Box::new(Type::Base(BaseType::Any)))));
        }
        _ => panic!("expected FuncDef"),
    }
}
...
fn all_base_types_parsed() {
    let stmts = parse("func f(a: I8, b: I16, c: I32, d: I64, e: U8, f: U16, g: U32, h: U64, i: F32, j: F64, k: String, l: Bool, m: Null, n: Void, o: Any) {}");
    match &stmts[0] {
        Stmt::FuncDef { params, .. } => {
            ...
            assert_eq!(params[14].ty, Some(Type::Base(BaseType::Any)));
        }
        _ => panic!("expected FuncDef"),
    }
}
