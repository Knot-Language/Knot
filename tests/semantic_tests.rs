...
#[test]
fn any_compatible_with_all_types() {
    assert!(check::types_compatible(&Type::Base(BaseType::Any), &Type::Base(BaseType::I32)));
    assert!(check::types_compatible(&Type::Base(BaseType::Any), &Type::Base(BaseType::String)));
    assert!(check::types_compatible(&Type::Base(BaseType::I32), &Type::Base(BaseType::Any)));
    assert!(check::types_compatible(&Type::Base(BaseType::Any), &Type::Base(BaseType::Bool)));
}

#[test]
fn any_wider_type_returns_any() {
    use knot::parser::ast::*;

    let mut sym = SymbolTable::new();
    sym.declare("a".to_string(), Some(Type::Base(BaseType::I32)));
    sym.declare("b".to_string(), Some(Type::Base(BaseType::Any)));

    let expr = Expr::Binary {
        op: BinOp::Add,
        left: Box::new(Expr::Ident("a".to_string(), Span::new(1, 1))),
        right: Box::new(Expr::Ident("b".to_string(), Span::new(1, 1))),
        span: Span::new(1, 1),
    };

    assert_eq!(check_expr(&expr, &mut sym), Some(Type::Base(BaseType::Any)));
}
