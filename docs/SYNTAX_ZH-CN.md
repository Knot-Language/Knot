...
泛型允许函数、类、枚举接受类型参数，实现参数化多态。类型参数在调用处自动推导，
编译器为每个具体类型实例生成单态化副本。
...
`throw` 将异常值存入全局异常寄存器并跳转到最近的 catch 标签。`catch` 块通过 `CatchEntry` 加载异常值。支持嵌套 try/catch，每层 try 会保存并恢复自己的处理标签。
...
`import` 在编译时加载并内联另一个 Knot 源文件的内容。
...
| 形式 | 示例 | 说明 |
|------|------|------|
| **字符串字面量** | `import "utils.knot"` | 文件路径，相对于导入文件。会进行规范化处理。 |
| **裸标识符** | `import math` | 模块名，通过模块搜索路径解析。 |

### 别名 (`as`)

提供别名时，导入文件中的所有顶层定义可通过 `<别名>.` 命名空间访问。
不提供别名时，导入的符号直接合并到当前作用域中。
