...
9. [泛型](#9-泛型)
10. [Mixin](#10-mixin)
11. [Wrap 和装饰器](#11-wrap-和装饰器)
12. [模块和导入](#12-模块和导入)
13. [Extern——调用 C 代码](#13-extern让-knot-调用-c-代码)
14. [综合示例](#14-综合示例来个完整的)
15. [下一步](#15-下一步)
...
- `io.print_str(...)` — 打印字符串到屏幕
- `return 0` — 返回 0 表示正常退出

> **关于 `print()`**：Knot 没有内置 `print`。后续示例中 `print(x)` 是
> `io.print_str(x)` 的简写。需先 `import "std/io.knot" as io` 并设
...
| 文本 | `Array[Char]`（也就是 `String`） | `"hello"` |
| 单字符 | `Char` | `'a'` |
| 真假 | `Bool` | `true` / `false` |
| 空 | `Null` | `null` |
...
任何不是零的值都算"真"，所以你可以直接写 `if flag { ... }` 而不必写
`if flag == true`。
...
first = nums[0]           // 取第一个：1
nums[0] = 99              // 改第一个
len = nums.length         // 数组长度：5
```

所有元素必须是同一类型，`[1, 2, 3]` 的类型是 `Array[I32]`。

### 字典：键值对，查东西飞快

```knot
config = {"host": "localhost", "port": 8080}
caps = {a: 1, b: 2}      // 键也可以不加引号

port = config["port"]     // 取：8080
config["debug"] = true    // 加一项
```

`{"a": 1}` 的类型是 `Map[String, I32]`。
...
`a..b` 就是从 a（包含）到 b（不包含）的一个区间，主要用于 for 循环。
